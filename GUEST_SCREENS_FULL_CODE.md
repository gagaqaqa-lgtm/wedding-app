...
1. `app/(guest)/guest/(entry)/page.tsx` - ゲスト入口ページ
2. `app/(guest)/guest/(onboarding)/survey/page.tsx` - アンケート/レビューページ
3. `app/(guest)/guest/(main)/gallery/page.tsx` - ギャラリーページ
4. `components/PostWeddingThankYouCard.tsx` - 新郎新婦用レビューカードコンポーネント
5. `components/guest/Lightbox.tsx` - ライトボックスコンポーネント
6. `components/guest/OpeningModal.tsx` - オープニングモーダルコンポーネント
7. `components/DownloadWaitModal.tsx` - ダウンロード待機モーダルコンポーネント

---

## 1. app/(guest)/guest/(entry)/page.tsx
...
## 2. app/(guest)/guest/(onboarding)/survey/page.tsx
...
## 4. components/PostWeddingThankYouCard.tsx
...
## 5. components/guest/Lightbox.tsx
...
## 6. components/guest/OpeningModal.tsx
...
- `entry/page.tsx` には344-477行目に未使用のダッシュボードコードが残っていますが、`type Step` に `'dashboard'` が含まれていないため実行されません。
- `survey/page.tsx` は `localStorage` チェックと「戻る」ボタンが削除された状態です（以前の実装から戻されました）。
- `gallery/page.tsx` は2378行の大きなファイルです。
