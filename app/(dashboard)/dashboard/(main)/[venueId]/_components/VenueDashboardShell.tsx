...
interface VenueDashboardShellProps {
  children: ReactNode;
  venue: Venue;
}

function VenueDashboardShellContent({ children, venue }: VenueDashboardShellProps) {
  ...
  // 契約プランの判定
  const isLightPlan = venue.plan === 'LIGHT';
  const planName = isLightPlan ? 'Light Plan' : venue.plan === 'PREMIUM' ? 'Premium Plan' : 'Standard Plan';
...
export function VenueDashboardShell({ children, venue }: VenueDashboardShellProps) {
  return (
    <NotificationProvider>
      <VenueDashboardShellContent venue={venue}>
        {children}
      </VenueDashboardShellContent>
    </NotificationProvider>
  );
}
