...
import { ReviewSettings } from "./_components/ReviewSettings";
...
const Icons = {
  ...
  Star: ({ className }: { className?: string }) => <svg className={className} width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>,
  Link: ({ className }: { className?: string }) => <svg className={className} width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"/></svg>,
};
...
const INITIAL_SETTINGS: SettingsData = {
  ...
  // 口コミ設定（デフォルト値）
  coupleReviewUrl: "",
  coupleReviewThreshold: 4,
  guestReviewUrl: "",
  guestReviewThreshold: 4,
...
const isValidEmail = (emailString: string): boolean => {
  if (!emailString.trim()) return true; // 空文字は有効（未入力状態）
  const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
  return emailRegex.test(emailString);
};

export default function VenueSettingsPage({ params }: VenueSettingsPageProps) {
  ...
  const hasChanges = useMemo(() => {
    return (
      ...
      settings.coupleReviewUrl !== originalSettings.coupleReviewUrl ||
      settings.coupleReviewThreshold !== originalSettings.coupleReviewThreshold ||
      settings.guestReviewUrl !== originalSettings.guestReviewUrl ||
      settings.guestReviewThreshold !== originalSettings.guestReviewThreshold ||
      settings.reviewNotificationEmail !== originalSettings.reviewNotificationEmail
    );
  ...
  const isValid = useMemo(() => {
    ...
    const coupleReviewUrlValid = isValidUrl(settings.coupleReviewUrl);
    const guestReviewUrlValid = isValidUrl(settings.guestReviewUrl);
    const emailValid = isValidEmail(settings.reviewNotificationEmail);
    return googleMapsValid && lineUrlValid && coupleReviewUrlValid && guestReviewUrlValid && emailValid;
  }, [settings]);
      ...
      <div className="flex-1 overflow-auto bg-slate-50">
        <div className="max-w-4xl mx-auto">
          ...
          <form onSubmit={handleSave}>
              ...
              <div className="space-y-6">
                ...
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-2">
                    口コミ投稿先のURL (Review Link)
                  </label>
                  <div className="text-xs text-gray-500 mb-3 space-y-1">
                    <p>
                      ゲストに口コミ投稿を依頼する際のリンク先を設定します。
                    </p>
                    <p>
                      <strong>Googleマップ</strong>だけでなく、<strong>みんなのウェディング</strong>、<strong>ウエディングパーク</strong>、<strong>ゼクシィ</strong>など、現在注力している媒体のURLを入力してください。
                    </p>
                  </div>
                  <div className="flex items-start gap-3">
                    <div className="flex items-center justify-center w-10 h-10 rounded-xl bg-amber-50 text-amber-600 flex-shrink-0 mt-1">
            ...
            <ReviewSettings
              settings={{
                coupleReviewUrl: settings.coupleReviewUrl,
                coupleReviewThreshold: settings.coupleReviewThreshold,
                guestReviewUrl: settings.guestReviewUrl,
                guestReviewThreshold: settings.guestReviewThreshold,
                reviewNotificationEmail: settings.reviewNotificationEmail,
              }}
              onUpdate={(reviewSettings) => {
                setSettings((prev) => ({
                  ...prev,
                  ...reviewSettings,
                }));
