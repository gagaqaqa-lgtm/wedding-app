...
const VENUE_PASSWORDS: Record<string, string> = {
  'venue-001': 'venue001',
  'venue-002': 'venue002',
  'venue-003': 'venue003',
};
...
export default function VenueLoginPage() {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    // 簡易認証ロジック
    const expectedPassword = VENUE_PASSWORDS[venueId];
    
    if (expectedPassword && password === expectedPassword) {
      // 成功時の処理
      const venueName = getVenueName(venueId);
      toast.success('ログイン成功', {
        description: `${venueName} のダッシュボードへ遷移します`,
        duration: 2000,
      });
        ...
        >
          ...
          >
            ...
            <ul className="space-y-1">
              {Object.entries(VENUE_PASSWORDS).map(([id, password]) => {
                const venueName = getVenueName(id);
                return (
                  <li key={id}>
                    ID: <code className="bg-white/80 px-1 rounded">{id}</code> / PASS: <code className="bg-white/80 px-1 rounded">{password}</code> ({venueName})
                  </li>
                );
              })}
            </ul>
